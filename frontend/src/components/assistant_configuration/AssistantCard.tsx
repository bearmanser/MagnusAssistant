...
export function AssistantCard({
  id,
  data,
  deleteAssistantFromData,
}: AssistantCardProps) {
  const [useCustomWakeWord, setUseCustomWakeWord] = useState<boolean>(
    data.wake_word !== "magnus"
  );
  ...
  useEffect(() => {
    GetVoices().then((r) => {
      if (r.error) {
        ...
        const initialVoice = voicesArray.find(
          (v) => v.key.split(".onnx")[0] === config.voice
        );
            ...
            <Center>
              ...
              >
                {voices &&
                  [
                    ...new Set(
                      Object.values(voices).map(
                        (voice: any) => voice.language.name_english
                      )
                    ),
                  ].map((uniqueLanguage, index) => (
              ...
              {useCustomWakeWord ? (
                ...
                >
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      e.target.files &&
                        setWakeWordFiles(Array.from(e.target.files));
          ...
          <Box>
            ...
            <Input
              ...
              onChange={(e) => {
                setConfig((prev) => ({
                  ...prev,
                  wake_word_sensitivity: parseFloat(e.target.value),
                }));
              }}
              ...
              {!useCustomVoice ? (
                  ...
                  <Select
                    ...
                    onChange={(e: ChangeEvent<HTMLSelectElement>) => {
                      const selectedVoice = voices.find(
                        (voice) => voice.key === e.target.value
                      );
                      setVoice(selectedVoice);
                      setConfig((prev) => ({
                        ...prev,
                        voice: selectedVoice?.key.split(".onnx")[0],
                      }));
                    }}
                        ...
                        (_voice: any, key: number) =>
                          _voice["language"]["name_english"] === language && (
                            <option key={key} value={_voice["key"]}>
                              {_voice["name"]} (
                              {_voice["language"]["country_english"]}) -{" "}
                              {_voice["quality"]} quality
                            </option>
                          )
                      )}
                  </Select>
                ...
                >
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      e.target.files && setVoiceOnnxFile(e.target.files[0])
                    }
                  />
                  {"json:"}
                  <input
                    ...
                    onChange={(e) =>
                      e.target.files && setVoiceJsonFile(e.target.files[0])
                    }
                  />
                </Flex>
              )}
